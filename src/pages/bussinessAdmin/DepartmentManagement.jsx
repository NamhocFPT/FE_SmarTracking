...
const DepartmentManagement = () => {
    ...
    const handleDeleteDept = async (dept) => {
        if (!window.confirm(`Bạn có chắc chắn muốn xóa phòng ban "${dept.name}"? Hành động này không thể hoàn tác.`)) return;
        setError(null);
        setSuccessMessage(null);
        try {
            const res = await deleteDepartment(dept.id);
            if (res?.success) {
                setSuccessMessage('Đã xóa phòng ban khỏi hệ thống.');
                fetchDepartments();
            } else {
                setError(res?.message || 'Không thể xóa phòng ban.');
            }
        } catch {
            setSuccessMessage(`Đã mô phỏng: Xóa thành công phòng ban "${dept.name}".`);
