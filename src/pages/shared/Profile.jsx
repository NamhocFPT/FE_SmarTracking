 ...
 * UC-AM-10: Xem chi tiết hồ sơ tài khoản
 * UC-AM-12: Cập nhật thông tin cá nhân (self)
 * Tích hợp liên kết đăng ký khuôn mặt (UC-AM-13) cho Employee/Manager
 */
const Profile = () => {
    const navigate = useNavigate();
    const location = useLocation();

    // Determine role and configuration from current URL path
    ...
    const fetchProfileData = useCallback(async () => {
        ...
        try {
            ...
            // Retrieve full profile details from database
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                avatarFileId: formData.avatarFile ? 'mock-uploaded-file-id' : undefined
            };
            ...
            if (res?.success) {
                ...
                if (userStr) {
                    ...
                    if (formData.avatarPreview) {
                        user.avatarUrl = formData.avatarPreview;
                    }
                ...
                setProfile(prev => ({
                    ...
                    avatarUrl: formData.avatarPreview || prev.avatarUrl
                }));
            }
        } catch (err) {
        ...
        <div className="space-y-6 animate-fade-in-up max-w-5xl mx-auto">
            <div>
                ...
                <p className="text-slate-blue text-sm mt-1">
                    Xem chi tiết thông tin hồ sơ của bạn và cấu hình sinh trắc học FaceID.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Left: Avatar & FaceID registration status */}
                <div className="md:col-span-1 bg-white p-6 rounded-2xl border border-platinum-tint shadow-sm-2 flex flex-col items-center text-center">
                    ...
                    <div className="flex flex-wrap gap-1.5 justify-center mt-3">
                        <span className="inline-flex text-[10px] px-2.5 py-0.5 bg-blue-50 text-action-blue rounded-full font-bold">
                            {roleLabel}
                        </span>
                    </div>
                    ...
                    <div className="w-full flex flex-col gap-3.5 p-3.5 bg-cloud-mist rounded-xl border border-outline-gray/60">
                        ...
                        {showFaceRegisterBtn && (
                            <button
                                type="button"
                                onClick={() => navigate(`${rolePath}/face-register`)}
                        ...
                        <div className="flex items-center justify-between border-b border-platinum-tint/60 pb-4 mb-4">
                            ...
                            {!editMode ? (
                                <button
                                    ...
                                    className="inline-flex items-center justify-center px-3 py-1.5 border border-platinum-tint bg-white text-action-blue hover:bg-cloud-mist rounded-lg text-xs font-semibold transition-all"
                                >
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className="px-3 py-1.5 border border-platinum-tint bg-white text-slate-blue hover:bg-cloud-mist rounded-lg text-xs font-semibold transition-all"
                                    >
                                        Hủy
                                    </button>
                                    <button
                                        ...
                                        disabled={updating}
                                        className="px-3 py-1.5 bg-action-blue hover:bg-glacier-blue text-white rounded-lg text-xs font-semibold shadow-sm transition-all flex items-center justify-center"
                                    >
                        ...
                        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full px-3 py-2 border rounded-xl text-sm transition-all focus:outline-none ${
                                        editMode 
                                            ? 'border-platinum-tint focus:border-action-blue bg-white text-midnight-indigo' 
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div className="p-4 rounded-xl bg-blue-50/50 border border-blue-100/70 flex items-start gap-3.5 transition-all hover:shadow-sm">
                                ...
                                <div>
                                    ...
                                    <span className="text-sm font-bold text-midnight-indigo mt-1 block">
                                        {profile.department?.departmentName || profile.department?.name || profile.departments?.[0]?.name || profile.departments?.[0]?.departmentName || 'Chưa phân bổ phòng ban'}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="p-4 rounded-xl bg-amber-50/45 border border-amber-100/70 flex items-start gap-3.5 transition-all hover:shadow-sm">
                                ...
                                <div>
                                    ...
                                    <span className="text-sm font-bold text-midnight-indigo mt-1 block">
                                        {profile.positionTitle || 'Chưa thiết lập chức danh'}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="p-4 rounded-xl bg-purple-50/50 border border-purple-100/70 flex items-start gap-3.5 transition-all hover:shadow-sm">
                                ...
                                <div>
                                    ...
                                    <span className="text-sm font-bold text-midnight-indigo mt-1 block">
                                        {profile.directManager?.fullName || 'Không có quản lý'}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-left">
                            <div>
                                ...
                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold mt-1.5 ${
                                    ...
                                    {profile.accountStatus === 'active' ? 'Hoạt động' : 'Bị khóa'}
                                </span>
                            </div>
                            <div>
                                <span className="block text-xs font-bold text-slate-blue uppercase">Đăng nhập cuối cùng</span>
                                <span className="text-xs font-semibold text-midnight-indigo mt-2.5 block">
                                    {profile.lastLoginAt ? new Date(profile.lastLoginAt).toLocaleString('vi-VN') : 'Chưa có thông tin'}
                                </span>
                            </div>
                            <div>
