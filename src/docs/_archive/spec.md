...
│                        ▼                                            │
│  ┌─────────────────────────────────────────────────────────┐        │
│  │  Infrastructure: Prisma ORM, Redis Client, Mail, S3 SDK │        │
│  └──┬────────────────┬──────────────┬────────────┬─────────┘        │
└─────┼────────────────┼──────────────┼────────────┼─────────────────┘
      ▼                ▼              ▼            ▼
 ┌──────────┐   ┌────────────┐  ┌─────────┐  ┌──────────┐
...
| Backend API | NestJS + Prisma + TypeScript | Business logic, REST, WebSocket, Ingestion | Trung tâm hệ thống; gom tất cả module nghiệp vụ |
...
| ADR-02 | ORM | Prisma | TypeORM, Sequelize | Type-safe, migration tốt, doc rõ |
...
Single PostgreSQL database, chia thành các schema theo bounded context để tránh trộn bảng. Migration quản lý qua Prisma migrate + raw SQL migration cho các constraint Prisma chưa support (exclude constraint, partial index).
...
**Migration raw SQL (Prisma không generate được, phải viết tay):**
